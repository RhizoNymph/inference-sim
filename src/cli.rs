...
struct ServingRejectionGroupKey {
    phase: String,
    category: String,
    resource: String,
    code: String,
...
struct ServingRejectionGroupSummary {
    key: ServingRejectionGroupKey,
    candidate_count: usize,
    rejection_count: usize,
    example_candidate_ids: Vec<String>,
...
struct ServingRejectionSummary {
    rejected_candidate_count: usize,
    candidate_with_rejections_count: usize,
    total_rejection_count: usize,
    groups: Vec<ServingRejectionGroupSummary>,
}
...
struct CliArgs {
    ...
    top_k: usize,
    request_metrics_csv_path: Option<PathBuf>,
    request_lifecycle_events_csv_path: Option<PathBuf>,
    serving_metrics_csv_path: Option<PathBuf>,
    serving_metric_breakdowns_csv_path: Option<PathBuf>,
    serving_services_csv_path: Option<PathBuf>,
    serving_utilization_csv_path: Option<PathBuf>,
    serving_memory_pressure_csv_path: Option<PathBuf>,
    serving_timeline_csv_path: Option<PathBuf>,
    serving_occupancy_csv_path: Option<PathBuf>,
    serving_placement_evidence_csv_path: Option<PathBuf>,
    serving_worker_evidence_csv_path: Option<PathBuf>,
    serving_rejections_csv_path: Option<PathBuf>,
    serving_route_paths_csv_path: Option<PathBuf>,
    kv_route_resources_csv_path: Option<PathBuf>,
    serving_bottlenecks_csv_path: Option<PathBuf>,
    serving_phase_calibration_csv_path: Option<PathBuf>,
    serving_approximations_csv_path: Option<PathBuf>,
    calibration_residuals_csv_path: Option<PathBuf>,
    scenario_sensitivity_csv_path: Option<PathBuf>,
    rank_sensitivity_csv_path: Option<PathBuf>,
    ...
    scenarios: Vec<RunScenarioConfig>,
}
...
impl OutputProfile {
    ...
    fn materialized_profiles(self) -> &'static [OutputProfile] {
        match self {
...
struct SearchDiagnostics {
    mode: SearchDiagnosticsMode,
    candidate_space_count: Option<usize>,
    prefill_candidate_space_count: Option<usize>,
    decode_candidate_space_count: Option<usize>,
    serving_pair_space_lower_bound_per_pool: Option<usize>,
    max_runtime_ms: Option<u64>,
    runtime_elapsed_ms: u128,
    searched_candidate_count: usize,
    reported_candidate_count: usize,
    omitted_rejected_candidate_count: usize,
    truncated_by_parallelism_budget: bool,
    truncated_by_prefill_budget: bool,
    truncated_by_decode_budget: bool,
    truncated_by_serving_pair_budget: bool,
    truncated_by_runtime_budget: bool,
    serving_pair_budget_exhausted: bool,
...
#[derive(Copy, Clone, Debug, PartialEq)]
struct CalibrationJsonContext<'a> {
    calibration: SimulationCalibration,
    policy: &'a CalibrationPolicy,
    profile: Option<&'a CalibrationProfileMetadata>,
    coverage: Option<&'a CalibrationCoverageReport>,
    warnings: &'a [CalibrationApplicabilityWarning],
    invalid_shape_warnings: &'a [CalibrationInvalidShapeWarning],
    gate_violations: &'a [CalibrationGateViolation],
}

#[derive(Clone, Debug, PartialEq)]
struct CalibrationUncertaintySummary {
    fit_count: usize,
    fit_count_with_uncertainty: usize,
    relative_uncertainty_pct: Option<f64>,
    absolute_uncertainty_s: Option<f64>,
    min_confidence_score: Option<f64>,
    max_extrapolation_ratio: Option<f64>,
    applicability_status: String,
}

#[derive(Clone, Debug, PartialEq, Eq)]
struct RankSensitivityApproximationSummary {
    status: String,
    approximation_count: usize,
    policy_violation_count: usize,
    coarse_topology: bool,
    approximate_queueing: bool,
    aggregate_memory: bool,
    uncalibrated_runtime: bool,
...
fn parallelism_search_budget(budget: RunSearchBudgetConfig) -> RunSearchBudgetConfig {
...
fn serving_search_budget(budget: RunSearchBudgetConfig) -> RunSearchBudgetConfig {
    RunSearchBudgetConfig {
        max_parallelism_candidates: budget.max_parallelism_candidates,
...
fn retain_rejected_candidates(budget: RunSearchBudgetConfig) -> bool {
    budget.retain_rejected_candidates.unwrap_or(true)
}

fn reported_parallelism_results(
...
fn rejected_candidate_count<T>(results: &[T], feasible: impl Fn(&T) -> bool) -> usize {
    results.iter().filter(|score| !feasible(score)).count()
}

fn search_space_candidate_count(search: &SearchSpace) -> usize {
    search
        .tensor_ranks
        .len()
...
fn effective_budgeted_count(candidate_count: usize, budget: Option<usize>) -> usize {
    budget
...
impl SearchRuntime {
    fn start(budget: RunSearchBudgetConfig) -> Self {
    ...
    fn truncated(self, searched_candidate_count: usize, budgeted_candidate_count: usize) -> bool {
        self.deadline_expired() && searched_candidate_count < budgeted_candidate_count
    }
}

fn parallelism_search_diagnostics(
    search: &SearchSpace,
    budget: RunSearchBudgetConfig,
    searched_candidate_count: usize,
    reported_candidate_count: usize,
...
fn serving_pair_space_lower_bound(
    serving: &DisaggregatedServingConfig,
    budget: RunSearchBudgetConfig,
) -> usize {
    let prefill_candidate_space_count = search_space_candidate_count(&serving.search.prefill);
...
fn effective_serving_pair_budgeted_count(
    serving: &DisaggregatedServingConfig,
    budget: RunSearchBudgetConfig,
) -> usize {
...
fn primary_output_filename(format: OutputFormat) -> &'static str {
    match format {
...
fn apply_output_profile_defaults(args: &mut CliArgs) {
...
fn apply_output_profile_paths(args: &mut CliArgs, profile_dir: &Path) {
    match args.output_profile {
        OutputProfile::Summary => {}
        OutputProfile::Compare => {
            set_output_path(
...
fn write_scenario_sweep<W: Write>(
    writer: &mut W,
    cluster: &Cluster,
...
struct ScenarioSensitivity {
    ...
    pool: Option<String>,
    hardware_unique_node_count: Option<u64>,
    hardware_unique_gpu_count: Option<u64>,
    hardware_prefill_gpu_count: Option<u64>,
    hardware_decode_gpu_count: Option<u64>,
    hardware_shared_gpu_count: Option<u64>,
    hardware_aggregate_hbm_gb: Option<f64>,
    hardware_prefill_hbm_gb: Option<f64>,
    hardware_decode_hbm_gb: Option<f64>,
    hardware_aggregate_effective_peak_tflops: Option<f64>,
    hardware_prefill_effective_peak_tflops: Option<f64>,
    hardware_decode_effective_peak_tflops: Option<f64>,
    hardware_aggregate_gpu_types: Option<String>,
    hardware_prefill_gpu_types: Option<String>,
    hardware_decode_gpu_types: Option<String>,
    hardware_aggregate_gpu_label_counts: Option<String>,
    hardware_prefill_gpu_label_counts: Option<String>,
    hardware_decode_gpu_label_counts: Option<String>,
    hardware_throughput_tokens_per_s_per_gpu: Option<f64>,
    hardware_throughput_tokens_per_s_per_effective_peak_tflop: Option<f64>,
    hardware_throughput_tokens_per_s_per_hbm_gb: Option<f64>,
    calibration_status: Option<String>,
    calibration_coverage_fraction: Option<f64>,
    calibration_fit_count: Option<u64>,
    calibration_fit_count_with_uncertainty: Option<u64>,
    calibration_relative_uncertainty_pct: Option<f64>,
    calibration_absolute_uncertainty_ms: Option<f64>,
    calibration_gate_violation_count: Option<u64>,
    calibration_hard_gate_violation_count: Option<u64>,
    approximation_status: Option<String>,
    approximation_count: Option<u64>,
    approximation_policy_violation_count: Option<u64>,
    approximation_calibration_count: Option<u64>,
    approximation_topology_count: Option<u64>,
    approximation_queueing_count: Option<u64>,
    approximation_coarse_topology: Option<bool>,
    approximation_approximate_queueing: Option<bool>,
    approximation_uncalibrated_runtime: Option<bool>,
    approximation_category_counts: Option<String>,
    approximation_top_codes: Option<String>,
    bottleneck_count: Option<u64>,
    top_bottleneck_source: Option<String>,
    top_bottleneck_category: Option<String>,
    top_bottleneck_code: Option<String>,
    top_bottleneck_severity: Option<String>,
    rejection_count: Option<u64>,
    top_rejection_phase: Option<String>,
    top_rejection_category: Option<String>,
    top_rejection_resource: Option<String>,
    top_rejection_code: Option<String>,
    top_rejection_unit: Option<String>,
    top_rejection_remediation: Option<String>,
    ttft_ms: Option<f64>,
    tpot_ms: Option<f64>,
...
fn scenario_metric_delta_values(
    value: Option<f64>,
    baseline: Option<f64>,
...
fn text_u32_list(values: &[u32]) -> String {
    if values.is_empty() {
        return "-".to_string();
    }
    values
...
fn apply_run_scenario_topology(
    cluster: &mut Cluster,
    scenario: &RunScenarioConfig,
) -> Result<(), CliError> {
...
fn apply_calibration_gates_to_parallelism(
...
fn apply_calibration_fit_gates_to_parallelism(
    results: &mut [ScoredParallelismConfig],
...
fn apply_calibration_gates_to_serving(
...
fn apply_calibration_fit_gates_to_serving(
    results: &mut [ScoredServingConfig],
    policy: &CalibrationPolicy,
) {
    for score in results {
...
fn apply_approximation_policy_to_parallelism(
    results: &mut [ScoredParallelismConfig],
...
fn apply_approximation_policy_to_serving(
    results: &mut [ScoredServingConfig],
    policy: &ApproximationPolicy,
) {
    for score in results {
...
fn sort_parallelism_results_after_policy(results: &mut [ScoredParallelismConfig]) {
    results.sort_by(compare_nominal_parallelism);
}

fn sort_serving_results_after_policy(results: &mut [ScoredServingConfig]) {
    results.sort_by(compare_nominal_serving);
}

fn approximation_policy_violations(
    policy: &ApproximationPolicy,
...
fn approximation_policy_action(
    policy: &ApproximationPolicy,
...
fn approximation_policy_violation_message(
    approximation: &SimulationApproximation,
    metric: Option<&str>,
) -> String {
    let metric = metric
...
fn serving_objective_metric_term(objective: ServingObjective) -> &'static str {
    match objective {
...
fn policy_term(value: &str) -> String {
    value.trim().to_ascii_lowercase().replace(['-', ' '], "_")
}

fn approximation_policy_reason(violations: &[ApproximationPolicyViolation]) -> String {
    violations
        .iter()
        .map(|violation| violation.message.as_str())
        .collect::<Vec<_>>()
...
fn calibration_fit_gate_violations(
    policy: &CalibrationPolicy,
...
fn calibration_phase_gate_violations(
    policy: &CalibrationPolicy,
...
fn has_non_empty_metadata(value: Option<&str>) -> bool {
    value.is_some_and(|value| !value.trim().is_empty())
}

fn has_fit_uncertainty_metadata(application: &CalibrationFitApplication) -> bool {
...
fn optional_count_label(value: Option<u32>) -> String {
    value
        .map(|value| value.to_string())
        .unwrap_or_else(|| "missing".to_string())
}

fn apply_uncertainty_adjusted_ranking_to_parallelism(
    results: &mut [ScoredParallelismConfig],
    policy: &CalibrationPolicy,
) {
...
fn apply_uncertainty_adjusted_ranking_to_serving(
    results: &mut [ScoredServingConfig],
    policy: &CalibrationPolicy,
) {
    let weight = policy.uncertainty_ranking_weight;
...
fn uncertainty_adjusted_parallelism_latency_s(score: &ScoredParallelismConfig, weight: f64) -> f64 {
...
fn compare_nominal_parallelism(
...
fn compare_uncertainty_adjusted_parallelism(
    a: &ScoredParallelismConfig,
    b: &ScoredParallelismConfig,
    weight: f64,
) -> Ordering {
...
fn parallelism_nominal_rank_map(results: &[ScoredParallelismConfig]) -> BTreeMap<String, usize> {
...
fn parallelism_uncertainty_adjusted_rank_map(
    results: &[ScoredParallelismConfig],
    weight: f64,
...
fn compare_uncertainty_adjusted_serving_objective(
...
fn uncertainty_adjusted_serving_objective_score(score: &ScoredServingConfig, weight: f64) -> f64 {
...
fn serving_uncertainty_adjusted_base_score(score: &ScoredServingConfig, weight: f64) -> f64 {
...
fn nominal_serving_objective_score(score: &ScoredServingConfig) -> f64 {
...
fn serving_objective_base_score(score: &ScoredServingConfig) -> f64 {
    match score.objective {
...
struct ServingObjectiveMetricDescriptor {
    metric: &'static str,
...
fn serving_objective_metric_descriptor(
...
fn serving_objective_penalty_score(score: &ScoredServingConfig) -> f64 {
    score.slo_miss_penalty_score
        + score.service_backpressure_penalty_score
        + score.topology_risk_penalty_score
}

fn serving_largest_nominal_objective_term(score: &ScoredServingConfig) -> &'static str {
    [
...
fn serving_objective_summary_note(score: &ScoredServingConfig) -> Option<String> {
...
fn optional_serving_objective_score(value: Option<f64>) -> f64 {
    value
        .filter(|value| value.is_finite())
...
fn serving_peak_memory_pressure_phase(score: &ScoredServingConfig) -> Option<&str> {
    score
        .memory_pressure
        .iter()
...
fn compare_nominal_serving(a: &ScoredServingConfig, b: &ScoredServingConfig) -> Ordering {
    b.feasible
        .cmp(&a.feasible)
        .then_with(|| {
...
fn compare_uncertainty_adjusted_serving(
    a: &ScoredServingConfig,
    b: &ScoredServingConfig,
    weight: f64,
) -> Ordering {
...
fn serving_nominal_rank_map(results: &[ScoredServingConfig]) -> BTreeMap<String, usize> {
    let mut ordered: Vec<_> = results.iter().collect();
...
fn serving_uncertainty_adjusted_rank_map(
    results: &[ScoredServingConfig],
    weight: f64,
) -> BTreeMap<String, usize> {
    let mut ordered: Vec<_> = results.iter().collect();
...
fn uncertainty_adjusted_latency_s(
    value_s: f64,
    applications: &[CalibrationFitApplication],
    weight: f64,
) -> f64 {
...
fn uncertainty_adjusted_phase_latency_s(
    value_s: f64,
    applications: &[CalibrationFitApplication],
    phase: &str,
    weight: f64,
...
fn serving_slo_miss_score(metrics: &ServingMetrics) -> f64 {
    [
...
fn hard_calibration_gate_reason(violations: &[CalibrationGateViolation]) -> Option<String> {
...
fn join_optional_reason(existing: Option<&str>, reason: &str) -> String {
    match existing {
...
fn calibration_gate_serving_rejection(violation: &CalibrationGateViolation) -> ServingRejection {
    ServingRejection {
...
fn approximation_policy_serving_rejection(
...
fn calibration_gate_resource(code: &str) -> &'static str {
...
fn calibration_gate_remediation(code: &str) -> &'static str {
    if code.contains("coverage") || code.contains("phase") {
...
fn write_calibration_gate_text<W: Write>(
    writer: &mut W,
    violations: &[CalibrationGateViolation],
) -> Result<(), io::Error> {
    if violations.is_empty() {
...
fn write_markdown_search_diagnostics<W: Write>(
    writer: &mut W,
...
fn write_markdown_cluster_inventory<W: Write>(
    writer: &mut W,
    cluster: &Cluster,
) -> Result<(), io::Error> {
    write_markdown_key_value_section(
...
fn write_markdown_serving_rejection_summary<W: Write>(
    writer: &mut W,
    results: &[ScoredServingConfig],
) -> Result<(), io::Error> {
...
fn write_cluster_inventory_text<W: Write>(
    writer: &mut W,
    cluster: &Cluster,
) -> Result<(), io::Error> {
    writeln!(
...
fn write_cluster_inventory<W: Write>(
    writer: &mut W,
    cluster: &Cluster,
    indent: &str,
    trailing_comma: bool,
...
fn write_approximation_policy<W: Write>(
    writer: &mut W,
    policy: &ApproximationPolicy,
    indent: &str,
    trailing_comma: bool,
...
fn write_search_budget<W: Write>(
    writer: &mut W,
    budget: RunSearchBudgetConfig,
    indent: &str,
    trailing_comma: bool,
...
fn write_search_diagnostics_text<W: Write>(
    writer: &mut W,
    diagnostics: SearchDiagnostics,
) -> Result<(), io::Error> {
    writeln!(
...
fn write_search_diagnostics<W: Write>(
    writer: &mut W,
    diagnostics: SearchDiagnostics,
    indent: &str,
    trailing_comma: bool,
...
fn write_calibration_gate_violation_array<W: Write>(
    writer: &mut W,
    violations: &[CalibrationGateViolation],
    indent: &str,
    field: &str,
...
fn write_calibration_uncertainty<W: Write>(
    writer: &mut W,
    indent: &str,
    field: &str,
    applications: &[CalibrationFitApplication],
...
fn write_serving_calibration_summary<W: Write>(
    writer: &mut W,
    indent: &str,
...
fn write_serving_phase_calibration<W: Write>(
    writer: &mut W,
    indent: &str,
    phases: &[ServingPhaseCalibrationObservation],
    trailing_comma: bool,
...
fn calibration_uncertainty_summary<'a>(
...
fn rank_sensitivity_approximation_summary(
    approximations: &[SimulationApproximation],
...
fn calibration_phase_uncertainty_summary<'a>(
    applications: impl Iterator<Item = &'a CalibrationFitApplication>,
    phase: &str,
) -> CalibrationUncertaintySummary {
...
fn write_calibration_fit_applications<W: Write>(
    writer: &mut W,
    indent: &str,
    field: &str,
    applications: &[CalibrationFitApplication],
...
fn write_optional_calibration_fit_application<W: Write>(
    writer: &mut W,
    indent: &str,
    field: &str,
...
#[derive(Copy, Clone, Debug, PartialEq)]
struct CalibrationBenchmarkLatencyResidual {
    error_ms: f64,
    abs_error_ms: f64,
    signed_pct_error: f64,
    abs_pct_error: f64,
}
...
fn calibration_benchmark_latency_residual(
...
fn calibration_benchmark_latency_residual_status(
...
fn write_serving_rejections<W: Write>(
    writer: &mut W,
    indent: &str,
    candidate_id: &str,
...
fn write_serving_rejection_summary_text<W: Write>(
    writer: &mut W,
    results: &[ScoredServingConfig],
) -> Result<(), io::Error> {
    let summary = serving_rejection_summary(results);
...
fn write_serving_rejection_summary_json<W: Write>(
    writer: &mut W,
    results: &[ScoredServingConfig],
    indent: &str,
    trailing_comma: bool,
...
fn serving_rejection_summary(results: &[ScoredServingConfig]) -> ServingRejectionSummary {
...
fn serving_rejection_summary_from_records<'a, I>(records: I) -> ServingRejectionSummary
where
...
fn json_optional_uncertainty_ms(feasible: bool, seconds: Option<f64>) -> String {
    if feasible {
...
fn json_optional_seconds_ms(seconds: Option<f64>) -> String {
    seconds.map(json_ms).unwrap_or_else(|| "null".to_string())
}

fn json_metric_uncertainty_ms(
    feasible: bool,
    value_s: f64,
...
fn json_metric_relative_uncertainty_value(
    feasible: bool,
    value: f64,
    uncertainty: &CalibrationUncertaintySummary,
) -> String {
...
fn json_metric_relative_lower_value(
    feasible: bool,
    value: f64,
    uncertainty: &CalibrationUncertaintySummary,
) -> String {
...
fn json_metric_relative_upper_value(
    feasible: bool,
    value: f64,
    uncertainty: &CalibrationUncertaintySummary,
) -> String {
...
fn metric_relative_uncertainty_value(
    value: f64,
    uncertainty: &CalibrationUncertaintySummary,
) -> Option<f64> {
    uncertainty
...
fn metric_relative_sensitivity_bounds(
    value: f64,
    uncertainty: &CalibrationUncertaintySummary,
) -> Option<(f64, f64)> {
