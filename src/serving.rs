...
mod tests {
    ...
    fn calibration_profile_with_fits(
    ...
    fn prefill_worker_slots_control_independent_prefill_overlap() {
        ...
        let mut single_slot_states =
            vec![pending_prefill_state(0, gpu), pending_prefill_state(1, gpu)];
    ...
    fn ranks_disaggregated_serving_configs_with_metrics() {
        ...
        assert!(
            results.iter().any(|score| {
                score.deployment_mode == ServingDeploymentMode::FullyDisaggregated
            })
        );
        assert!(
            results
        ...
        assert!(
            results[0].metrics.decode_iteration_p90_s >= results[0].metrics.decode_iteration_p50_s
        );
        assert!(
            results[0].metrics.decode_iteration_p95_s >= results[0].metrics.decode_iteration_p50_s
        );
        assert!(
            results[0].metrics.decode_iteration_max_s >= results[0].metrics.decode_iteration_p99_s
        );
        ...
        assert!(
            results[0].metrics.peak_allocated_kv_tokens >= results[0].metrics.peak_resident_tokens
        );
        assert!(
        ...
        assert!(
            results[0].metric_breakdowns.iter().any(|breakdown| {
                breakdown.group == "prefill_node" && breakdown.key == "node-0"
            })
        );
        assert!(
            results[0]
        ...
        assert!(results[0].metric_breakdowns.iter().any(|breakdown| {
            breakdown.group == "prefill_route" && breakdown.key == "nodes[0]"
        }));
        assert!(
            results[0].metric_breakdowns.iter().any(|breakdown| {
                breakdown.group == "decode_route" && breakdown.key == "nodes[1]"
            })
        );
        assert!(
            results[0]
        ...
        assert!(
            results[0].approximations.iter().any(|approximation| {
                approximation.code == "approximate_kv_residency_accounting"
            })
        );
        ...
        let runtime_profile = CalibrationProfileMetadata {
            ...
            serving_runtime_features: vec![
                "paged_attention".to_string(),
                "cuda_graphs".to_string(),
            ],
            backend_version: None,
            driver_version: None,
            cuda_version: None,
        ...
        assert!(profiled[0].approximations.iter().any(|approximation| {
            approximation.phase == "kv_transfer"
                && approximation.code == "serving_phase_uncalibrated"
        }));
        ...
        let covered_runtime_features =
            vec!["paged_attention".to_string(), "cuda_graphs".to_string()];
    ...
    fn slo_miss_penalty_changes_latency_objective_ordering() {
        ...
        fast_with_misses.slo_miss_penalty_score =
            fast_with_misses.slo_miss_penalty_components.total;
        ...
        slower_without_misses.slo_miss_penalty_components =
            slo_miss_penalty_components_from_metrics(
    ...
    fn topology_bottlenecks_report_domain_concentration_risk() {
        ...
        let bottlenecks = topology_domain_bottleneck_observations(
            &cluster,
            &prefill_placement,
            &decode_placement,
        );
    ...
    fn serving_memory_uses_configured_kv_block_tokens_for_block_table_overhead() {
        ...
        assert!(
            (small_blocks[0].decode_memory.components.block_table_gb - expected_small_gb).abs()
                < 1e-15
        );
        assert!(
            (large_blocks[0].decode_memory.components.block_table_gb - expected_large_gb).abs()
                < 1e-15
        );
    }
    ...
    fn kv_transfer_worker_slots_report_worker_queue() {
        ...
        assert!(
            (observations[0].prefill_finish_s - observations[1].prefill_finish_s).abs() < 1e-12
        );
        assert!(
            observations
                .iter()
